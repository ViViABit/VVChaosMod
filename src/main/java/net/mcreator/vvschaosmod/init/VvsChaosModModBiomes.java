...
import net.mcreator.vvschaosmod.world.biome.MeatbiomeBiome;
...
public class VvsChaosModModBiomes {
	public static final DeferredRegister<Biome> REGISTRY = DeferredRegister.create(ForgeRegistries.BIOMES, VvsChaosModMod.MODID);
	public static final RegistryObject<Biome> MEATBIOME = REGISTRY.register("meatbiome", () -> MeatbiomeBiome.createBiome());
			...
			for (Map.Entry<ResourceKey<LevelStem>, LevelStem> entry : worldGenSettings.dimensions().entrySet()) {
				...
				if (dimensionType == dimensionTypeRegistry.getOrThrow(DimensionType.OVERWORLD_LOCATION)) {
					...
					if (chunkGenerator.getBiomeSource() instanceof MultiNoiseBiomeSource noiseSource) {
						...
						parameters.add(new Pair<>(MeatbiomeBiome.PARAMETER_POINT,
								biomeRegistry.getOrCreateHolder(ResourceKey.create(Registry.BIOME_REGISTRY, MEATBIOME.getId()))));
						parameters.add(new Pair<>(MeatbiomeBiome.PARAMETER_POINT_UNDERGROUND,
								biomeRegistry.getOrCreateHolder(ResourceKey.create(Registry.BIOME_REGISTRY, MEATBIOME.getId()))));
					...
					if (chunkGenerator instanceof NoiseBasedChunkGenerator noiseGenerator) {
						...
						if (currentRuleSource instanceof SurfaceRules.SequenceRuleSource sequenceRuleSource) {
							...
							surfaceRules.add(1, anySurfaceRule(ResourceKey.create(Registry.BIOME_REGISTRY, MEATBIOME.getId()),
									VvsChaosModModBlocks.MEAT_BLOCK.get().defaultBlockState(), VvsChaosModModBlocks.MUSCLE.get().defaultBlockState(),
									VvsChaosModModBlocks.VEIN.get().defaultBlockState()));
							surfaceRules.add(1, preliminarySurfaceRule(ResourceKey.create(Registry.BIOME_REGISTRY, MEATBIOME.getId()),
									VvsChaosModModBlocks.MEAT_BLOCK.get().defaultBlockState(), VvsChaosModModBlocks.MUSCLE.get().defaultBlockState(),
									VvsChaosModModBlocks.VEIN.get().defaultBlockState()));
